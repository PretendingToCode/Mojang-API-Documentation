...
## api.mojang.com/user/parentalconsent ##
...
Response: 204 successful regardless of parental consent status

Description: Returns whether a user has recieved parental consent or not
...
Body: JSON object with the following fields
```
parentalconsent: Boolean - Whether to set parental consent to true or false
```

Response: Usually returns "Parental consent request failed"

Description: Sets a user's parental consent status
...
## api.mojang.com/user/profile/[UUID]/rewards ##
...
Response: JSON object with the following fields
```
rewards: Array - List of claimed rewards(?)
comingRewards: Array - List of pending rewards(?)
upcomingRewards: Array - List of approved rewards(?)
```

Description: Returns information about an authenticated user's rewards
...
Body: JSON object with the following fields
```
Rewards: Array - Array containing strings that are reward IDs
```

Response: Array of JSON objects with the following fields
```
rewardId: String - Reward ID submitted in array
successful: Boolean - Whether the submitted rewardId was valid
```
...
Response: Array of JSON objects with the following fields
```
answer: Object - Contains fields relating to security answer
answer.id: Int - ID of security answer
question: Object - Contains fields relating to security question
question.id: Int - ID of security question
question.question: String - Security question
```

Description: Returns list of security questions, or empty array if account is not secured
...
Body: Array of JSON objects with the following fields
```
id: Int - Answer ID
